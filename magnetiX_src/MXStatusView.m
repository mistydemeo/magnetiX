...
@implementation MXStatusView


- (void)drawRect:(NSRect)rect
{	
	float lowerBorder = [[self window] contentBorderThicknessForEdge:NSMinYEdge];
	
	if (rect.origin.y < lowerBorder) {
		lowerBorder -= rect.origin.y;
		rect.origin.y += lowerBorder;
		rect.size.height -= lowerBorder;
	}
...
{
	[visibleScrollView scrollWheel:theEvent];
}
